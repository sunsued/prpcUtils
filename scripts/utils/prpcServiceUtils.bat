...
if "%PEGA_TOOL%"=="updateDASS" goto :pegaToolOK
...
exit /b 0
:--processFailStrategy
set "ANT_PROPS=%ANT_PROPS% -Dservice.processFailStrategy=%1"
exit /b 0
:ArgError
...
:PrintUsage
echo usage:
echo prpcserviceutils.bat ( expose ^| export ^| hotfix ^| getStatus ^| import ^| rollback ^| manageRestorePoints ^| updateAccessGroup ^| updateDASS ^| help ) [options...]
echo.
echo Options:
